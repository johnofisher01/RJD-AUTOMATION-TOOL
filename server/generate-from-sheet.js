  ...
  try {
    ...
    if (JOB_FILTER) {
      filtered = rows.filter(r => {
        ...
        const mapped = mapSheetRowToTemplateFields(r);
        return norm(mapped.JOB_NO) === norm(JOB_FILTER);
      });
      if (!filtered.length) {
        console.log(`No rows matched job filter="${JOB_FILTER}"`);
        return;
      }
      console.log(`Rows matched job filter="${JOB_FILTER}":`, filtered.length);
    }
